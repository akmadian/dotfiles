...
char* genEntry(char* dir, char* fname, int depth) {
  ...
  if (strcmp(fname, ".") == 0 || strcmp(fname, "..") == 0)
    ...
    if (strcmp(ftype, "directory") == 0 && !FULLSIZE) {
      formattedSize = "____";
    } else {
    formattedSize = sizeString(fullname);
    }
...
void printhelp(char* name) {
  fprintf(stderr, "Usage: %s [-hdvtf] [directory...]\n", name);
  fprintf(stderr, "    -h  Print this help message.\n");
  fprintf(stderr, "    -d  Debug mode, prints args and such.\n");
  fprintf(stderr, "    -v  Verbose mode. Doesn't do anything for now.\n");
  fprintf(stderr, "    -t  Print tree from dir.\n");
  fprintf(stderr, "    -f  Recurse into dirs to get their full size.\n");
  exit(EXIT_FAILURE);
}
